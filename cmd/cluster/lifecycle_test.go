...
func setupLifecycleTest(t *testing.T) (*testutils.RecordingTimer, *testutils.StubFactory, *testutils.StubProvisioner, shared.LifecycleConfig) {
t.Helper()
...
t.Fatalf("expected StartCount=1, NewStageCount=0, got %d/%d", timer.StartCount, timer.NewStageCount)
}
}
...
t.Fatalf("expected NewStageCount=1, CallCount=1, got %d/%d", timer.NewStageCount, factory.CallCount)
}
}
...
t.Fatalf("expected StartCount=1, NewStageCount=1, got %d/%d", timer.StartCount, timer.NewStageCount)
}
