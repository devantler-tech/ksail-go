...
- **Download dependencies**: `go mod download` (completes in ~0.04 seconds when cached, up to 30 seconds on first run)
- **Build the application**: `go build -o ksail .` -- takes ~0.52 seconds when dependencies cached, ~8 seconds on first build. Set timeout to 60+ seconds for safety.
...
- **mockery**: For generating mocks: Install per [mockery docs](https://vektra.github.io/mockery/v3.5/installation/)
  ...
  - **Helper Script**: Use `bash scripts/run-mockery.sh` for automated mockery execution with compatibility checks
- **golangci-lint**: Alternative linting tool: `curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh | sh -s -- -b ~/go/bin latest`
  ...
  - **Current Environment**: Available at `~/go/bin/golangci-lint` - takes ~2 minutes to run
...
- **Run tests**: `go test -v ./...` -- takes ~35 seconds. Set timeout to 120+ seconds for safety.
- **Generate mocks**: `mockery` -- regenerates test mocks in ~1.3 seconds. Required when interface definitions change.
  - **Alternative**: Use `bash scripts/run-mockery.sh` for automated execution with compatibility checks
- **Run linter**: `mega-linter-runner -f go` -- comprehensive linting with Go flavor as specified in CONTRIBUTING.md
  ...
  - **Go-specific linting**: Run `~/go/bin/golangci-lint run` separately for Go-specific checks (takes ~2 minutes, shows 0 issues)
  ...
  - **Note**: Mega-linter may report some non-critical issues like external link timeouts or code duplication detection
...
4. **MANDATORY Full Workflow Test**: After any changes, run this complete validation:
   ...
   go test -v ./...                    # ~35 seconds - ALL tests must pass
   go build -o ksail .                 # ~0.52 seconds - must build successfully
   ...
   # Optional Go-specific linting: ~/go/bin/golangci-lint run (takes ~2 minutes, shows 0 issues)
...
- **`go build -o ksail .`**: ~0.52 seconds when cached, ~8 seconds first time -- SET TIMEOUT TO 60+ SECONDS for safety
- **`go test -v ./...`**: ~35 seconds -- SET TIMEOUT TO 120+ SECONDS for safety
...
- **`~/go/bin/golangci-lint run`**: ~2 minutes (alternative linter) -- SET TIMEOUT TO 180+ SECONDS for safety
- **`go mod download`**: ~0.04 seconds when cached, up to 30 seconds first run -- SET TIMEOUT TO 60+ SECONDS for safety
...
- **Performance**: For faster iteration during development, use `~/go/bin/golangci-lint run` for Go-specific linting (~2 minutes)
...
- **`scripts/`**: Build and development helper scripts
  - `run-mockery.sh` - Automated mockery execution with compatibility checks
