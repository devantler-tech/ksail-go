...
- Q: What is the exposure scope of the unauthenticated local OCI registry? → A: No auth, localhost-only (workstation + cluster), not reachable from other machines.
- Q: Where should administrators primarily see Flux reconciliation status and errors? → A: Only via kubectl/Flux CR status; ksail shows exit code.
...
- **FR-001**: System MUST support installing Flux controllers during cluster bootstrap using the Flux Installer (Go SDK) when Flux is specified as the GitOps engine
- **FR-002**: System MUST support installing Flux controllers on-demand to existing clusters using the Flux Installer
- **FR-003**: System MUST provision a local OCI-compliant registry container (using image `registry:3`) during cluster creation when registry provisioning is enabled
...
- **FR-006**: System MUST support building OCI artifacts from directories containing Kubernetes manifests (Kustomize-compatible) using native Go libraries (e.g., `google/go-containerregistry`)
- **FR-007**: System MUST support pushing OCI artifacts to repositories in the local registry with semantic versioning using native Go libraries, following the naming pattern `oci://<registry-endpoint>/<project-name>`
- **FR-008**: System MUST generate Flux OCIRepository custom resources that reference repositories in the local registry using the `oci://<registry-endpoint>/<project-name>` pattern
- **FR-009**: System MUST generate Flux Kustomization custom resources that source manifests from OCIRepository resources (HelmRelease resources are out of scope)
...
- **FR-013**: System MUST expose Flux reconciliation status and any errors encountered during synchronization via Flux custom resource status and conditions, inspectable using `kubectl` and Flux-native tooling (ksail MAY surface only high-level success/failure codes)
...
- **OCI Registry**: A local container registry instance (image `registry:3`) storing OCI artifacts, accessible from both workstation and cluster, with persistent storage volumes and configurable port mappings
